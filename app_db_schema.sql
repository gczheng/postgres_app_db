...
-- =====================================================
-- App_db数据库结构脚本 (v3.1.0)
-- 数据库名称: app_db
-- 字符集: UTF8
-- 版本: v3.1.0
-- 创建日期: 2026-04-14
-- 修订日期: 2026-06-09
-- 说明: 包含3个Schema：mall(电商核心)、audit(审计日志)、gis(地理位置)，移除了所有外键约束。重构了相关说明并移除了文件名版本后缀。
-- =====================================================
--
-- 使用方法（2026-06-09 修订）：
--   方式 A（最常用）：先在外部 createdb app_db，然后连 app_db 跑此脚本
--       $ createdb app_db
--       $ psql -d app_db -v dbname=app_db -f app_db_schema.sql
--   方式 B：让此脚本自己 CREATE DATABASE（取消下面 CREATE DATABASE 段的注释）
--       $ psql -d postgres -v dbname=app_db -f app_db_schema.sql
--
-- 关键修复（卡点 #1）：默认注释 CREATE DATABASE + \c，避免重入失败
-- 关键修复（卡点 #2）：CREATE EXTENSION postgis_topology 之前先
--     ALTER DATABASE ... SET search_path 加入 gis schema，
--     否则 CREATE EXTENSION 不使用 session search_path，找不到 geometry
-- =====================================================
...
-- =====================================================
-- 创建数据库（默认注释，由调用方先 createdb 完成；如要让此脚本自己建库，
-- 取消下面 5 行注释并用 psql -d postgres -f ... 执行）
-- =====================================================
...
-- 卡点 #2 修复：在装 postgis_topology 之前先把 gis schema 加到当前库的默认
-- search_path，否则 postgis_topology 内部脚本引用 geometry 时找不到类型
-- （CREATE EXTENSION 不使用 SET search_path 的 session 设置）
-- 调用方需通过 -v dbname=<库名> 传入数据库名，默认 app_db
\if :{?dbname}
\else
\set dbname app_db
\endif
ALTER DATABASE :"dbname" SET search_path TO mall, audit, gis, public, topology;
...
-- =====================================================
-- 设置默认搜索路径（session 级别；上面 ALTER DATABASE 已写入元数据）
-- =====================================================
...
-- 用户表序列
CREATE SEQUENCE IF NOT EXISTS mall.users_user_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
-- 地址表序列
CREATE SEQUENCE IF NOT EXISTS mall.addresses_address_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
-- 用户资料表序列
CREATE SEQUENCE IF NOT EXISTS mall.user_profiles_profile_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
-- 分类表序列
CREATE SEQUENCE IF NOT EXISTS mall.categories_category_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
-- 商品表序列
CREATE SEQUENCE IF NOT EXISTS mall.products_product_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
-- 商品图片表序列
CREATE SEQUENCE IF NOT EXISTS mall.product_images_image_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
-- 订单表序列
CREATE SEQUENCE IF NOT EXISTS mall.orders_order_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
-- 订单项表序列
CREATE SEQUENCE IF NOT EXISTS mall.order_items_order_item_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
-- 订单状态历史表序列
CREATE SEQUENCE IF NOT EXISTS mall.order_status_history_history_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
-- 支付表序列
CREATE SEQUENCE IF NOT EXISTS mall.payments_payment_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
-- 评论表序列
CREATE SEQUENCE IF NOT EXISTS mall.reviews_review_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
-- 地址表
CREATE TABLE IF NOT EXISTS mall.addresses (
...
-- 用户资料表
CREATE TABLE IF NOT EXISTS mall.user_profiles (
...
-- 分类表
CREATE TABLE IF NOT EXISTS mall.categories (
...
-- 商品图片表
CREATE TABLE IF NOT EXISTS mall.product_images (
...
-- 订单项表
CREATE TABLE IF NOT EXISTS mall.order_items (
...
-- 订单状态历史表
CREATE TABLE IF NOT EXISTS mall.order_status_history (
...
-- 支付表
CREATE TABLE IF NOT EXISTS mall.payments (
...
-- 评论表
CREATE TABLE IF NOT EXISTS mall.reviews (
...
-- 用户表索引
CREATE INDEX IF NOT EXISTS idx_users_email ON mall.users(email);
COMMENT ON INDEX idx_users_email IS '用户邮箱索引，用于快速查找';
CREATE INDEX IF NOT EXISTS idx_users_username ON mall.users(username);
COMMENT ON INDEX idx_users_username IS '用户名索引，用于快速查找';
CREATE INDEX IF NOT EXISTS idx_users_is_active ON mall.users(is_active);
COMMENT ON INDEX idx_users_is_active IS '用户状态索引，用于筛选激活用户';
CREATE INDEX IF NOT EXISTS idx_users_created_at ON mall.users(created_at);
COMMENT ON INDEX idx_users_created_at IS '用户创建时间索引，用于按时间排序';

-- 地址表索引
CREATE INDEX IF NOT EXISTS idx_addresses_user_id ON mall.addresses(user_id);
COMMENT ON INDEX idx_addresses_user_id IS '地址用户ID索引，用于快速查询用户地址';

-- 用户资料表索引
CREATE INDEX IF NOT EXISTS idx_user_profiles_user_id ON mall.user_profiles(user_id);
COMMENT ON INDEX idx_user_profiles_user_id IS '用户资料用户ID索引，用于快速查询用户资料';

-- 分类表索引
CREATE INDEX IF NOT EXISTS idx_categories_parent_category_id ON mall.categories(parent_category_id);
COMMENT ON INDEX idx_categories_parent_category_id IS '分类父ID索引，用于查询子分类';
CREATE INDEX IF NOT EXISTS idx_categories_is_active ON mall.categories(is_active);
COMMENT ON INDEX idx_categories_is_active IS '分类状态索引，用于筛选启用分类';
CREATE INDEX IF NOT EXISTS idx_categories_sort_order ON mall.categories(sort_order);
COMMENT ON INDEX idx_categories_sort_order IS '分类排序索引，用于按排序查询';

-- 商品表索引
CREATE INDEX IF NOT EXISTS idx_products_category_id ON mall.products(category_id);
COMMENT ON INDEX idx_products_category_id IS '商品分类ID索引，用于查询分类商品';
CREATE INDEX IF NOT EXISTS idx_products_name ON mall.products(name);
COMMENT ON INDEX idx_products_name IS '商品名称索引，用于商品搜索';
CREATE INDEX IF NOT EXISTS idx_products_is_active ON mall.products(is_active);
COMMENT ON INDEX idx_products_is_active IS '商品状态索引，用于筛选上架商品';
CREATE INDEX IF NOT EXISTS idx_products_price ON mall.products(price);
COMMENT ON INDEX idx_products_price IS '商品价格索引，用于价格排序和筛选';
CREATE INDEX IF NOT EXISTS idx_products_created_at ON mall.products(created_at);
COMMENT ON INDEX idx_products_created_at IS '商品创建时间索引，用于按时间排序';

-- 商品图片表索引
CREATE INDEX IF NOT EXISTS idx_product_images_product_id ON mall.product_images(product_id);
COMMENT ON INDEX idx_product_images_product_id IS '商品图片商品ID索引，用于查询商品图片';

-- 订单表索引
CREATE INDEX IF NOT EXISTS idx_orders_user_id ON mall.orders(user_id);
COMMENT ON INDEX idx_orders_user_id IS '订单用户ID索引，用于查询用户订单';
CREATE INDEX IF NOT EXISTS idx_orders_address_id ON mall.orders(address_id);
COMMENT ON INDEX idx_orders_address_id IS '订单地址ID索引，用于查询地址订单';
CREATE INDEX IF NOT EXISTS idx_orders_status ON mall.orders(status);
COMMENT ON INDEX idx_orders_status IS '订单状态索引，用于筛选订单状态';
CREATE INDEX IF NOT EXISTS idx_orders_created_at ON mall.orders(created_at);
COMMENT ON INDEX idx_orders_created_at IS '订单创建时间索引，用于按时间排序';
CREATE INDEX IF NOT EXISTS idx_orders_order_number ON mall.orders(order_number);
COMMENT ON INDEX idx_orders_order_number IS '订单编号索引，用于快速查找订单';

-- 订单项表索引
CREATE INDEX IF NOT EXISTS idx_order_items_order_id ON mall.order_items(order_id);
COMMENT ON INDEX idx_order_items_order_id IS '订单项订单ID索引，用于查询订单详情';
CREATE INDEX IF NOT EXISTS idx_order_items_product_id ON mall.order_items(product_id);
COMMENT ON INDEX idx_order_items_product_id IS '订单项商品ID索引，用于查询商品销售记录';

-- 订单状态历史表索引
CREATE INDEX IF NOT EXISTS idx_order_status_history_order_id ON mall.order_status_history(order_id);
COMMENT ON INDEX idx_order_status_history_order_id IS '订单历史订单ID索引，用于查询订单状态历史';
CREATE INDEX IF NOT EXISTS idx_order_status_history_created_at ON mall.order_status_history(created_at);
COMMENT ON INDEX idx_order_status_history_created_at IS '订单历史创建时间索引，用于按时间排序';

-- 支付表索引
CREATE INDEX IF NOT EXISTS idx_payments_order_id ON mall.payments(order_id);
COMMENT ON INDEX idx_payments_order_id IS '支付订单ID索引，用于查询订单支付';
CREATE INDEX IF NOT EXISTS idx_payments_status ON mall.payments(status);
COMMENT ON INDEX idx_payments_status IS '支付状态索引，用于筛选支付状态';
CREATE INDEX IF NOT EXISTS idx_payments_created_at ON mall.payments(created_at);
COMMENT ON INDEX idx_payments_created_at IS '支付创建时间索引，用于按时间排序';
CREATE INDEX IF NOT EXISTS idx_payments_transaction_id ON mall.payments(transaction_id);
COMMENT ON INDEX idx_payments_transaction_id IS '支付流水号索引，用于快速查找支付';

-- 评论表索引
CREATE INDEX IF NOT EXISTS idx_reviews_product_id ON mall.reviews(product_id);
COMMENT ON INDEX idx_reviews_product_id IS '评论商品ID索引，用于查询商品评论';
CREATE INDEX IF NOT EXISTS idx_reviews_user_id ON mall.reviews(user_id);
COMMENT ON INDEX idx_reviews_user_id IS '评论用户ID索引，用于查询用户评论';
CREATE INDEX IF NOT EXISTS idx_reviews_rating ON mall.reviews(rating);
COMMENT ON INDEX idx_reviews_rating IS '评论评分索引，用于按评分排序';
CREATE INDEX IF NOT EXISTS idx_reviews_created_at ON mall.reviews(created_at);
...
CREATE INDEX IF NOT EXISTS idx_audit_logs_operation_time ON audit.audit_logs(operation_time);
...
CREATE INDEX IF NOT EXISTS idx_login_logs_user_id ON audit.login_logs(user_id);
COMMENT ON INDEX idx_login_logs_user_id IS '登录日志用户ID索引，用于查询用户登录记录';

CREATE INDEX IF NOT EXISTS idx_login_logs_login_time ON audit.login_logs(login_time);
COMMENT ON INDEX idx_login_logs_login_time IS '登录日志登录时间索引，用于按时间查询';

CREATE INDEX IF NOT EXISTS idx_data_change_history_table_name ON audit.data_change_history(table_name);
COMMENT ON INDEX idx_data_change_history_table_name IS '数据变更历史表名索引，用于按表查询变更';

CREATE INDEX IF NOT EXISTS idx_data_change_history_record_id ON audit.data_change_history(record_id);
COMMENT ON INDEX idx_data_change_history_record_id IS '数据变更历史记录ID索引，用于查询特定记录的变更';

CREATE INDEX IF NOT EXISTS idx_data_change_history_change_time ON audit.data_change_history(change_time);
...
-- 门店位置表
CREATE TABLE IF NOT EXISTS gis.store_locations (
...
-- 配送区域表
CREATE TABLE IF NOT EXISTS gis.delivery_zones (
...
-- 物流轨迹表
CREATE TABLE IF NOT EXISTS gis.logistics_tracks (
...
-- 热点区域表
CREATE TABLE IF NOT EXISTS gis.hotspot_areas (
    hotspot_id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
-- PostGIS空间索引
CREATE INDEX IF NOT EXISTS idx_store_locations_geometry ON gis.store_locations USING GIST(geometry);
COMMENT ON INDEX idx_store_locations_geometry IS '门店位置空间索引，用于空间查询';

CREATE INDEX IF NOT EXISTS idx_delivery_zones_geometry ON gis.delivery_zones USING GIST(geometry);
COMMENT ON INDEX idx_delivery_zones_geometry IS '配送区域空间索引，用于空间查询';

CREATE INDEX IF NOT EXISTS idx_logistics_tracks_geometry ON gis.logistics_tracks USING GIST(geometry);
COMMENT ON INDEX idx_logistics_tracks_geometry IS '物流轨迹空间索引，用于空间查询';

CREATE INDEX IF NOT EXISTS idx_hotspot_areas_geometry ON gis.hotspot_areas USING GIST(geometry);
COMMENT ON INDEX idx_hotspot_areas_geometry IS '热点区域空间索引，用于空间查询';

-- 普通索引
CREATE INDEX IF NOT EXISTS idx_store_locations_is_active ON gis.store_locations(is_active);
COMMENT ON INDEX idx_store_locations_is_active IS '门店状态索引，用于筛选营业门店';

CREATE INDEX IF NOT EXISTS idx_logistics_tracks_order_id ON gis.logistics_tracks(order_id);
COMMENT ON INDEX idx_logistics_tracks_order_id IS '物流轨迹订单ID索引，用于查询订单轨迹';

CREATE INDEX IF NOT EXISTS idx_logistics_tracks_start_time ON gis.logistics_tracks(start_time);
